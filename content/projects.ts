...
export const projects: readonly Project[] = [
  ...
  },
  {
    ...
    title: "ProxiVision",
    description: {
      es: "Lentes inteligentes de asistencia para personas con discapacidad visual. Un sensor ultrasónico independiente en cada patilla, sobre ESP32 con MicroPython, detecta obstáculos a la altura de la cabeza (ramas, letreros, postes) y avisa con vibración por conducción ósea en cada lado, modulada según la distancia para indicar hacia dónde esquivar, sin ocupar el oído. Incluye app móvil en React Native y una API serverless en Vercel para telemetría y configuración.",
      en: "Assistive smart glasses for visually impaired people. An independent ultrasonic sensor on each temple, running on ESP32 with MicroPython, detects head-level obstacles (branches, signs, poles) and warns through bone-conduction vibration on each side, modulated by distance to signal which way to dodge, without blocking hearing. Includes a React Native mobile app and a serverless Vercel API for telemetry and configuration.",
      de: "Assistive Smart Glasses für sehbehinderte Menschen. Ein unabhängiger Ultraschallsensor an jedem Bügel, auf ESP32 mit MicroPython, erkennt Hindernisse in Kopfhöhe (Äste, Schilder, Pfosten) und warnt per Knochenschall-Vibration auf jeder Seite, nach Distanz moduliert, um die Ausweichrichtung anzuzeigen – ohne das Gehör zu blockieren. Mit React-Native-App und serverloser Vercel-API für Telemetrie und Konfiguration.",
      it: "Occhiali intelligenti assistivi per persone con disabilità visiva. Un sensore a ultrasuoni indipendente su ciascuna astina, su ESP32 con MicroPython, rileva ostacoli all'altezza della testa (rami, cartelli, pali) e avvisa con vibrazione a conduzione ossea su ogni lato, modulata in base alla distanza per indicare da che parte scansare, senza occupare l'udito. Include app mobile in React Native e API serverless su Vercel per telemetria e configurazione.",
      nl: "Assistieve slimme bril voor mensen met een visuele beperking. Een onafhankelijke ultrasone sensor op elk pootje, op ESP32 met MicroPython, detecteert obstakels op hoofdhoogte (takken, borden, palen) en waarschuwt met beengeleidingstrilling aan elke kant, gemoduleerd op afstand om aan te geven welke kant op te ontwijken, zonder het gehoor te blokkeren. Met React Native-app en serverloze Vercel-API voor telemetrie en configuratie.",
    },
    tags: ["ESP32", "MicroPython", "Ultrasonic", "React Native", "Wearable", "Vercel"],
    links: {
